# Test unitari per core/llm (integrazione LLM).
#
# Copre le funzioni pure di sokoban_prompt (griglia_a_testo, crea_prompt, parsifica_azione,
# conta_casse) senza bisogno di connettività verso il LLM. I test di ClienteLLM.chiedi()
# vengono saltati automaticamente se Ollama non è attivo su localhost:11434, così la suite
# resta verde anche su macchine senza GPU o senza il server locale.
#
...
def griglia_vuota() -> np.ndarray:
    """Griglia 10x10 tutta muri (valore 0), per testare i casi limite."""
...
def griglia_semplice() -> np.ndarray:
    ...
    Il pavimento riempie la zona interna (bordo escluso); le celle di interesse sono in
    posizioni note, per semplificare le asserzioni.
    """
    g = np.zeros((10, 10), dtype=np.float32)
    g[1:-1, 1:-1] = 1.0   # pavimento interno (il bordo resta muro)
    g[5, 5] = 5.0          # giocatore
...
def griglia_con_casse_su_target() -> np.ndarray:
    """
    Griglia con 2 casse già su target (valore 4) e 1 cassa ancora libera.
    Serve a verificare che conta_casse distingua le casse piazzate da quelle da spostare.
    """
...
class TestGrigliaATesto:
    ...
    def test_simboli_corretti(self, griglia_semplice):
        """Le celle significative devono mappare sui simboli attesi in SIMBOLI."""
        testo = griglia_a_testo(griglia_semplice)
        righe = testo.split("\n")
        # Giocatore in (5,5)
        ...
        # Muro in (0,0): il bordo deve restare muro
...
class TestCreaPrompt:
    ...
    def test_contiene_griglia(self, griglia_semplice):
        """Il testo della griglia deve essere incluso nel prompt, per dare contesto visivo."""
        txt = griglia_a_testo(griglia_semplice)
        prompt = crea_prompt(txt, casse_su_target=0, n_casse=1)
    ...
    def test_contiene_azioni_valide(self, griglia_semplice):
        """Il prompt deve elencare le 4 azioni valide, così il LLM sa cosa rispondere."""
        txt = griglia_a_testo(griglia_semplice)
        prompt = crea_prompt(txt, casse_su_target=0, n_casse=1)
...
class TestParsificaAzione:
    ...
    def test_parole_valide(self, testo, atteso):
        """Tutte le varianti previste del vocabolario devono mappare sull'azione giusta."""
    ...
    def test_fallback_su_testo_invalido(self):
        """
        Un testo sconosciuto deve restituire comunque un intero valido in {0,1,2,3}.
        Il fallback casuale va bene: l'importante è non crashare e non uscire dallo spazio
        delle azioni.
        """
    ...
    def test_stringa_vuota_fallback(self):
        """La stringa vuota è un caso limite che non deve sollevare eccezioni."""
...
class TestContaCasse:
    def test_nessuna_cassa(self, griglia_vuota):
        """Su una griglia tutta-muri non c'è alcuna cassa."""
        su_t, tot = conta_casse(griglia_vuota)
        assert su_t == 0
        assert tot == 0

    def test_cassa_su_target(self, griglia_con_casse_su_target):
        """Deve contare separatamente le casse su target (4) e quelle libere (3)."""
...
def _ollama_attivo() -> bool:
    """
    Indica se il server Ollama risponde su localhost:11434.
    Usa urllib puro per non aggiungere dipendenze a runtime; il timeout breve (2s) evita
    attese lunghe in CI, dove Ollama non gira.
    """
...
class TestClienteLLM:
    def test_istanziazione_ollama(self):
        """ClienteLLM('ollama') deve istanziarsi senza errori quando Ollama è attivo."""
    ...
    def test_chiedi_timeout_non_crasha(self):
        """
        Con un timeout quasi nullo il client deve restituire stringa vuota, non crashare.
        Il contratto è che ClienteLLM gestisca i timeout internamente, restituendo "" invece
        di propagare l'eccezione al chiamante.
        """
        from core.llm import ClienteLLM
        c = ClienteLLM("ollama")

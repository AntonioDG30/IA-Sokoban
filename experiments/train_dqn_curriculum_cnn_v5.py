"""Script di training AG-DQN v5 con curriculum learning + CnnPolicy + padding distinto (Opzione B).

Identico a train_dqn_curriculum_cnn.py (v4) ma con padding=7 (PADDING) invece di
0 (MURO), cosi' la CNN puo' distinguere il bordo artificiale dai muri reali.

Struttura curriculum (v3, invariata):
    C0: 5x5,  1 cassa, 300k step
    C1: 5x5,  2 casse, 400k step
    C2: 7x7,  3 casse, 400k step
    C3: 10x10, 4 casse, 300k step (Boxoban)

Nota: DQN non supporta ambienti paralleli (n_envs=1 fisso).

Uso:
    python experiments/train_dqn_curriculum_cnn_v5.py --seed 42
    python experiments/train_dqn_curriculum_cnn_v5.py --seed 42 --solo_valuta

Modelli salvati in: models/dqn_v5/
"""
...
def addestra_curriculum(seme: int, solo_valuta: bool) -> None:
    """Esegue il training curriculum DQN-CNN v5 per un singolo seed."""
    dir_dati = str(DIR_DATI) if DIR_DATI.exists() else None
    dir_output = str(DIR_MODELLI / "dqn_v5")
    percorso = percorso_modello_dqn_v5(seme)

    if solo_valuta:
        if not percorso.with_suffix(".zip").exists():
            print(f"[train_dqn_v5] Modello non trovato: {percorso}.zip")
            return
    ...
    for i, fase in enumerate(FASI_CURRICULUM):
        ...
        print(f"\n{'=' * 60}")
        print(f"[DQN-CNN-v5] Seed {seme} | {nome_fase} | {timestep_fase:,} step")
        ...
        ckpt_cb = CheckpointCallback(
            ...
            name_prefix=f"dqn_cnn_v5_{nome_fase}_seed{seme}",
            verbose=0,
        )
        ...
        modello.learn(
            ...
            callback=callbacks,
            tb_log_name=f"DQN_CNN_V5_seed{seme}",
            reset_num_timesteps=prima_fase,
            progress_bar=False,
        )

        print(f"[DQN-CNN-v5] {nome_fase} completata.")

    modello.save(str(percorso))
    print(f"\n[DQN-CNN-v5] Modello finale salvato: {percorso}.zip")
    _valuta_finale(modello, dir_dati, seme)
...
def _valuta_finale(modello, dir_dati, seme: int) -> None:
    ...
    for diff, split in [("unfiltered", "test"), ("medium", "valid")]:
        ...
        if dir_dati is None or not test_path.exists():
            print(f"[DQN-CNN-v5] {diff}/{split} non trovato, saltato.")
            continue
        ...
        env.close()
        print(
            f"\n[AgenteDQN-v5] Valutazione ({diff}/{split}, {n_ep} episodi):\n"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training DQN v5: curriculum + CnnPolicy + padding distinto (Opzione B)."
    )
...
def main() -> None:
    ...
    print("=" * 60)
    print("AG-DQN v5 -- Curriculum + CnnPolicy + Padding Distinto (B)")
    ...
    print("\n[train_dqn_curriculum_cnn_v5] Tutti i seed completati.")

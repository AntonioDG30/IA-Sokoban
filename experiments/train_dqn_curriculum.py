"""Script di training AG-DQN v2 con curriculum learning (Fase 2.3).

Addestra DQN in 3 fasi progressive con griglie crescenti:
...
I pesi del modello DQN vengono preservati tra le fasi (set_env).
...
Uso:
    python experiments/train_dqn_curriculum.py
    python experiments/train_dqn_curriculum.py --seed 42
    python experiments/train_dqn_curriculum.py --seed 42 --solo_valuta

Modelli salvati in: models/dqn_v2/
"""
...
def addestra_curriculum(seme: int, solo_valuta: bool) -> None:
    """Esegue il training curriculum DQN per un singolo seed."""
    dir_dati = str(DIR_DATI) if DIR_DATI.exists() else None
    dir_output = str(DIR_MODELLI / "dqn_v2")
    percorso = percorso_modello_dqn_v2(seme)

    if solo_valuta:
        if not percorso.with_suffix(".zip").exists():
            print(f"[train_dqn_curriculum] Modello non trovato: {percorso}.zip")
            return
    ...
    config = {k: v for k, v in CONFIG_DQN.items() if k != "tensorboard_log"}
    modello = None
    ...
    for i, fase in enumerate(FASI_CURRICULUM):
        ...
        print(f"\n{'=' * 60}")
        print(f"[DQN Curriculum] Seed {seme} | {nome_fase} | {timestep_fase:,} step")
        ...
        if prima_fase:
            # Prima fase: crea il modello DQN
            modello = DQN(
                env=env_train,
                seed=seme,
        ...
        ckpt_cb = CheckpointCallback(
            ...
            name_prefix=f"dqn_curriculum_{nome_fase}_seed{seme}",
            verbose=0,
        )
        ...
        modello.learn(
            ...
            callback=callbacks,
            tb_log_name=f"DQN_curriculum_seed{seme}",
            reset_num_timesteps=prima_fase,
            progress_bar=False,
        )

        print(f"[DQN Curriculum] {nome_fase} completata.")

    # Salva modello finale
    modello.save(str(percorso))
    print(f"\n[DQN Curriculum] Modello finale salvato: {percorso}.zip")
...
def _valuta_finale(modello, dir_dati, seme: int) -> None:
    ...
    for diff, split in [("unfiltered", "test"), ("medium", "valid")]:
        ...
        if dir_dati is None or not test_path.exists():
            print(f"[DQN Curriculum] {diff}/{split} non trovato, saltato.")
            continue
        ...
        import numpy as np
        print(
            f"\n[AgenteDQN-v2] Valutazione ({diff}/{split}, {n_ep} episodi):\n"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Training DQN v2 con curriculum learning (Fase 2.3)."
    )
...
def main() -> None:
    ...
    print("=" * 60)
    print("AG-DQN v2 -- Curriculum Learning + Manhattan Shaping")
    ...
    print("\n[train_dqn_curriculum] Tutti i seed completati.")

# Training di AG-LLM-GUIDE sul curriculum 7x7 (C0->C2) — gemello di train_llm_guide.py.
#
# Learning from Demonstrations con il LLM come guida. Per ogni fase:
#   1. RACCOLTA DEMO: il LLM gioca N_DEMO_LLM_7x7 episodi salvando le transizioni
...
#
# Identico a src/sistema_10x10/train_llm_guide.py tranne che per: l'ambiente SokobanEnv7x7 (7,7), 3
# fasi generate C0/C1/C2 invece di 6 e l'assenza di dataset Boxoban. Architettura DQN come
# AG-DQN del 7x7 (train_dqn_7x7.py): CnnPolicy con SokobanCNN (features_dim=256), AggiuntaCanale
# (7,7)->(1,7,7) e stesse soglie del curriculum adattivo.
#
# Uso: python src/sistema_7x7/train_llm_guide_7x7.py [--seed 42] [--provider ollama]
# Modello finale salvato in artifacts/models/7x7/llm_guide/llm_guide_7x7_seed{seed}.zip
...
def _leggi_max_solve_rate(dir_eval_logs: Path) -> float:
    """Legge dai log di valutazione SB3 il massimo solve rate (%) raggiunto (soglia 9.0)."""
...
def addestra_curriculum(seed: int, provider: str) -> None:
    """
    Addestra AG-LLM-GUIDE con il curriculum LfD 7x7 C0->C2.
    Stessa logica di train_llm_guide.py ma con SokobanEnv7x7 (7,7), 3 fasi generate invece di
    6 e nessuno split "valid" (solo livelli generati).
    """
    ...
    # Agente guida LLM (come in train_llm_guide.py)
    ...
    for i_fase, fase in enumerate(FASI_CURRICULUM_7x7):
        ...
        # AggiuntaCanale: (7,7) -> (1,7,7), come in train_llm_guide.py
        ...
        # Ambienti DQN di training e validazione (come in train_llm_guide.py)
        ...
        # FASE 2: crea o aggiorna il modello DQN (come in train_llm_guide.py)
        if modello is None:
            modello = DQN(
                policy="CnnPolicy",
                env=env_train,
        ...
        # Callback (come in train_llm_guide.py)
        ...
        # FASE 3: training DQN con curriculum adattivo (come in train_llm_guide.py)

# Training di AG-DQN sul curriculum 7x7 (C0->C2) — gemello di train_dqn.py.
#
# DQN (CnnPolicy) con SokobanCNN su griglia nativa 7x7. Identico a src/sistema_10x10/train_dqn.py
# tranne che per: l'ambiente SokobanEnv7x7 (7,7) al posto di SokobanEnv (10,10), 3 fasi
# generate C0/C1/C2 invece di 6, e il budget scalato (~67%).
#
...
#
# Uso: python src/sistema_7x7/train_dqn_7x7.py [--seed 42]
# Modello finale salvato in artifacts/models/7x7/dqn/dqn_7x7_seed{seed}.zip
...
def _leggi_max_solve_rate(dir_eval_logs: Path) -> float:
    """
    Legge dai log SB3 il massimo solve rate (%) raggiunto. Come in train_dqn.py, usa la
...
def addestra_curriculum(seed: int) -> None:
    """
    Addestra AG-DQN con il curriculum 7x7 C0->C2.
    Stessa logica di train_dqn.py::addestra_curriculum() ma con SokobanEnv7x7 (7,7), 3 fasi
    ...
    # Policy kwargs: stessa CNN custom di train_dqn.py
    policy_kwargs = dict(
    ...
    for i_fase, fase in enumerate(FASI_CURRICULUM_7x7):
        ...
        # Ambienti di training e validazione (singoli, Monitor).
        ...
        if modello is None:
            ...
            modello.set_env(env_train)   # buffer conservato tra le fasi
        ...
        # Curriculum adattivo (stessa logica di train_dqn.py)
